...
public class Fireball extends Sprite {
	...
	public Fireball(PlayScreen screen, float x, float y, boolean fireRight){
		...
		for(int i = 0; i < 4; i++){
			frames.add(new TextureRegion(screen.getAtlas().findRegion("Bola de Fogo"), i * 8, 0, 8, 8));
		}
		fireAnimation = new Animation(0.2f, frames);
		setRegion((TextureRegion)fireAnimation.getKeyFrame(0));
		setBounds(x, y, 6 / ClasseGame.PPM, 6 / ClasseGame.PPM);
		defineFireBall();
	}

	public void defineFireBall(){
		BodyDef bdef = new BodyDef();
		bdef.position.set(fireRight ? getX() + 12 /ClasseGame.PPM : getX() - 12 /ClasseGame.PPM, getY());
		...
		shape.setRadius(3 / ClasseGame.PPM);
		fdef.filter.categoryBits = ClasseGame.FIREBALL_BIT;
